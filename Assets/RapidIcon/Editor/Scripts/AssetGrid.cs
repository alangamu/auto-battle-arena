...
namespace RapidIconUIC
{
	...
	string[] filterNames = new string[] { "Prefabs & Models", "Prefabs Only", "Models Only" };
		...
		void DrawIcons(float gridWidth)
		{
			...
			foreach (KeyValuePair<string, Icon> icon in sortedIconsByPath)
			{
				...
				if (icon.Value.previewRender == null)
				{
					...
					icon.Value.previewRender = Utils.RenderIcon(icon.Value, previewSize, (int)(((float)icon.Value.exportResolution.y / (float)icon.Value.exportResolution.x) * previewSize), FilterMode.Point);
				}
		...
		Icon CreateIcon(ObjectPathPair objectPathPair)
		{
			...
			icon.previewRender = Utils.RenderIcon(icon, previewSize, (int)(((float)icon.exportResolution.y/ (float)icon.exportResolution.x) * previewSize), FilterMode.Point);
